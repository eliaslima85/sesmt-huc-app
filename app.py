...
if menu == "📊 Dashboard":
    ...
    if p_data.data:
        for r in p_data.data:
            ...
            col2.markdown(f'<a href="https://api.whatsapp.com/send?phone=55{r["oficiais"]["whatsapp"]}&text={msg}" target="_blank"><button style="background-color:#25D366; color:white; border:none; border-radius:5px; width:100%; cursor:pointer;">📲 REENVIAR ZAP</button></a>', unsafe_allow_html=True)
    else:
        st.info("Tudo em dia! Nenhuma assinatura pendente.")
...
# --- PÁGINA: FUNCIONÁRIOS ---
    ...
    # Busca os dados fresquinhos do Supabase
    ...
    v_data = supabase.table("vinculos").select("nome").order("nome").execute().data
    
    # Se achou dados, cria a lista. Se não achou, mostra aviso.
    s_list = [s['nome'] for s in s_data] if s_data else ["Cadastre em Configurações"]
    f_list = [f['nome'] for f in f_data] if f_data else ["Cadastre em Configurações"]
    v_list = [v['nome'] for v in v_data] if v_data else ["Cadastre em Configurações"]
    ...
    with t1:
        with st.form("cad_func", clear_on_submit=True):
            ...
            zap = st.text_input("WhatsApp (Ex: 85999999999)")
            vinc = st.selectbox("Vínculo", v_list)
            
            if st.form_submit_button("Salvar Funcionário"):
                if "Cadastre em" in setor:
                    st.error("⚠️ Você precisa rodar o SQL no Supabase para carregar os setores!")
                else:
                    try:
                        ...
                        st.error(f"❌ Erro ao salvar. A matrícula já existe?")
    ...
    with t2:
        ...
        if not df.empty:
            ...
            f_set = c2.multiselect("Filtrar por Setor", df['setor'].unique())
...
# --- PÁGINA: ENTREGAR EPI ---
    ...
    if not res_f.data or not res_e.data:
        ...
        if st.button("Registrar Entrega"):
            st.success("✅ Entrega registrada! Envie o link pelo Dashboard.")

# --- PÁGINA: CATÁLOGO DE EPI ---
elif menu == "📦 Catálogo":
    st.title("📦 Catálogo de EPIs")
    with st.form("cad_epi"):
        n_epi = st.text_input("Nome do EPI")
        ca_epi = st.text_input("C.A.")
        val_epi = st.date_input("Validade C.A.", format="DD/MM/YYYY")
        if st.form_submit_button("Salvar EPI"):
            try:
                ...
                st.success("✅ EPI salvo no catálogo!")
                st.rerun()
            except Exception as e:
                st.error("Erro ao salvar EPI.")
    
    df_ep = pd.DataFrame(supabase.table("ep").select("*").execute().data)
    if not df_ep.empty:
        df_ep['validade'] = df_ep['validade'].apply(format_br)
        st.dataframe(df_ep[['nome', 'ca', 'validade']], use_container_width=True)

# --- PÁGINA: FICHA DE EPI ---
elif menu == "📄 Ficha de EPI":
    st.title("📄 Ficha de EPI (Histórico Total)")
    df_f = pd.DataFrame(supabase.table("oficiais").select("*").execute().data)
    if not df_f.empty:
        sel = st.selectbox("Selecione o Funcionário", df_f['nome'], key="sel_ficha")
        ...
        if h_res.data:
            ...
            if dias >= 20: st.warning(f"⚠️ Atenção: Sem entregas há {dias} dias. Recomenda-se gerar nova ficha!")
...
# --- PÁGINA: CONSUMO SEMANAL ---
    ...
    if res_last.data:
        ...
        else: st.success(f"Balanço em dia. Próximo vencimento em {7 - atraso} dias.")
    st.info("Aqui você gera relatórios semanais por setor (Em desenvolvimento).")

# --- PÁGINA: CONFIGURAÇÕES ---
elif menu == "⚙️ Configurações":
    st.title("⚙️ Painel de Configurações")
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["🔗 Sistema", "🏢 Setores", "🛠️ Funções", "📋 Vínculos", "🔑 Segurança e Ficha"])
    
    with tab1:
        st.write("Link base do sistema para envio de WhatsApp:")
        st.code(url_base)
        
    with tab2:
        st.write("#### Setores Cadastrados")
        ns = st.text_input("Novo Setor", key="in_setor").upper()
    ...
    with tab3:
        ...
        nf = st.text_input("Nova Função", key="in_func").upper()
    ...
    with tab4:
        st.write("#### Vínculos Cadastrados")
        nv = st.text_input("Novo Vínculo", key="in_vinc").upper()
        if st.button("Salvar Vínculo", key="btn_vinc"):
            try:
                supabase.table("vinculos").insert({"nome": nv}).execute(); st.rerun()
            except: st.error("Erro: Vínculo já existe.")
    ...
    with tab5:
        ...
        st.write("#### Texto Legal (Rodapé da Ficha de EPI)")
        texto_ficha = st.text_area("Termos Jurídicos", get_config("ficha_descricao"), key="txt_ficha", height=150)
